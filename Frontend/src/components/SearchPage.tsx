            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2 my-3">
              ...
              <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
                {appliedFilter.map((selectedFilter: string, idx: number) => (
                  ...
                  >
                    <Badge
                      className="text-[#00BFFF] rounded-md hover:cursor-pointer pr-6 whitespace-nowrap"
                      variant="outline"
                    >
                      {selectedFilter}
                    </Badge>
                    <X
                      ...
                      size={16}
                      className="absolute text-[#00BFFF] right-1 hover:cursor-pointer"
                    />
                  </div>
                ))}
              </div>
                  ...
                  >
                    ...
                    <CardFooter className="p-4 border-t dark:border-t-gray-700 border-t-gray-100 text-white flex justify-end">
                      <Link to={`/restaurant/${restaurant._id}`}>
                        <Button className="bg-sky-blue hover:bg-sky-blue font-semibold py-2 px-4 rounded-full shadow-md transition-colors duration-200">
                          View Menus
                        </Button>
                      </Link>
                    </CardFooter>
...
const NoResultFound = ({ searchText }: { searchText: string }) => {
  return (
    <div className="text-center">
      ...
      <Link to="/">
        <Button className="mt-4 bg-sky-blue hover:bg-sky-blue">
          Go Back to Home
        </Button>
      </Link>
    </div>
