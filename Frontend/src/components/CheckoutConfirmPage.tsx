...
const CheckoutConfirmPage = ({
  ...
  const [restaurantNoteText, setRestaurantNoteText] = useState<string>(restaurantNote || "");
  ...
  const handleRestaurantNoteChip = (text: string) => {
    setRestaurantNoteText((prev) => (prev ? `${prev}, ${text}` : text));
  };
  ...
  const checkoutHandler = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const checkoutData: CheckoutSessionRequest = {
        ...
        restaurantNote: restaurantNoteText,
                  ...
                  <form onSubmit={checkoutHandler} className="space-y-4">
                    ...
                    <div className="space-y-1.5 pt-1">
                      <Label className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                        ...
                        Cooking Instructions / Restaurant Note (Optional)
                      </Label>
                      <Input
                        type="text"
                        placeholder="e.g. Make it less spicy, extra mint chutney, no onions"
                        value={restaurantNoteText}
                        onChange={(e) => setRestaurantNoteText(e.target.value)}
                        className="text-xs rounded-xl"
                      />
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {["Less spicy 🌶️", "Extra spicy 🔥", "No onions / garlic 🧅", "No cutlery needed 🍴", "Extra napkins 🧻", "Well cooked 🍳"].map((chip) => (
                          <button
                            key={chip}
                            type="button"
                            onClick={() => handleRestaurantNoteChip(chip)}
                            className="text-[11px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded-md hover:bg-orange-100 hover:text-orange-600 border border-slate-200 dark:border-slate-700 transition-colors"
                          >
                            + {chip}
                          </button>
